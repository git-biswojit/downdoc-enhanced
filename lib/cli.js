...
import { postProcessCsvToMdTable } from "./post_process_csv_table.js";
import { preProcessIndexAdoc } from "./pre_process_index-adoc.js";
...
async function processDirectory(inputDir, outputDir, opts) {
  if (opts?.readme) {
    await preProcessIndexAdoc(inputDir);
  }
  return await processDirectoryRecursive(inputDir, outputDir, opts);
}
...
async function convertFile(inputPath, opts, outputPath = toOutputPath(inputPath)) {
  ...
  let output = downdoc(tableProcessedInput, opts) + "\n";
  output = postProcessCsvToMdTable(output);
