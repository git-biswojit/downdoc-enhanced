...
async function run(p = process) {
  ...
  if (outputPath && inputPath !== "-") {
    await processDirectory.call(p, inputPath, outputPath, attributes && { attributes });
  } else if (isDirectory && inputPath !== "-") {
    if (values.output) {
      await processDirectory.call(
        p,
        inputPath,
        values.output,
      ...
      await processDirectory.call(p, inputPath, inputPath, attributes && { attributes });
    }
  } else {
...
function convertFile(inputPath, opts, outputPath = toOutputPath(inputPath)) {
  const write = (data) =>
    outputPath === "-"
      ...
      : fsp.writeFile(outputPath, data, "utf8");
  const read = () =>
...
function printUsage(options, error) {
  ...
  let usage = [
    ...
    "If OUTPUT_DIR is specified, process all .adoc files in FILE directory recursively.",
