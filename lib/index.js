import { applySubs } from "./functions/applySubs.js";
import { attributes } from "./functions/attributes.js";
import { hardbreak, isAnyListItem, isHeading } from "./functions/check-functions.js";
import { image } from "./functions/image.js";
import { getFootnotes, processFootnotes, resetFootnotes } from "./functions/macros.js";
import { parseAttrlist } from "./functions/parseAttrList.js";
import { writeBlockTitle } from "./functions/writeBlockTitle.js";
...
import {
  ...
  DELIMS,
  NORMAL_SUBS,
  PreprocessorDirectiveRx,
  RevisionInfoLineRx,
  RewriteInternalXrefRx,
  TDIV,
} from "./constants/constants.js";

export default function downdoc(asciidoc, { attributes: initialAttrs = {} } = {}) {
  // Don't include doctitle from API if there's an actual document title
  const { doctitle, ...otherAttrs } = initialAttrs;
  const attrs = new Map(Object.entries(Object.assign({}, ATTRIBUTES, otherAttrs)));
  const lines = asciidoc.split("\n");

  // Reset footnotes for new document
  resetFootnotes();

  // Preprocess lines to combine header cells on separate lines
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i];
    if (
      line.trim().startsWith("[") &&
      line.includes('options="header"') &&
      i + 1 < lines.length
    ) {
      ...
      if (tableStart !== -1) {
        // Look for header cells on separate lines
        let headerEnd = -1;
        ...
        if (headerEnd !== -1) {
          // Check if we have multiple consecutive lines starting with |
          const headerLines = [];
          for (let j = tableStart + 1; j < headerEnd; j++) {
            if (lines[j].trim().startsWith("|")) {
              headerLines.push(j);
            }
          }
          // If we have multiple header lines, combine them
          if (headerLines.length > 1) {
            const combinedHeader = lines[headerLines[0]].trim();
            const headerCells = [combinedHeader];
            for (let k = 1; k < headerLines.length; k++) {
              const cellLine = lines[headerLines[k]].trim();
              if (cellLine.startsWith("|")) {
                headerCells.push(cellLine);
              }
            }
            // Replace the first header line with combined header
            // Replace header line with the first row
            lines[headerLines[0]] = headerCells[0];
            // Insert remaining lines in order
            for (let k = 1; k < headerCells.length; k++) {
              lines.splice(headerLines[0] + k, 0, headerCells[k]);
            }
            // Remove the other header lines
            for (let k = headerLines.length - 1; k > 0; k--) {
              lines.splice(headerLines[k], 1);
            }
          }
        }
      }
  ...
  inHeader =
    (inContainer = inPara = (indent = "") || false) ||
    asciidoc[0] === "=" ||
    !!~asciidoc.indexOf("\n= ");
  let blockAttrs,
    blockTitle,
    ...
    style,
    verbatim;
  const [containerStack, nrefs, refs, skipStack, undef] = [
    (listStack = []).slice(),
    new Map(),
    new Map(),
    [],
      ...
      while ((grab = match = next = style = subs = undefined) === undefined) {
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === "endif::[]" && line))) {
          ...
          if (match[1]) {
            ...
            const name = match[4];
            const text = match[5];
            const drop = attrs.has(name) ? !!negated : !negated;
            if (text ? (drop ? false : (line = text)) : !skipStack.push(drop && "endif::[]")) continue; // redo
          } else if (match[3] === "include" && !inContainer.verbatim) {
            // Handle include directives by converting them to markdown links
            const includeTarget = match[4] || "";
            const linkText = includeTarget.replace(/\.(adoc|asciidoc)$/, "");
            line = `[${linkText}](${includeTarget}.md)`;
          }
        ...
        } else if (inContainer.admonition) {
          if (line === inContainer.delimiter) {
            ;({ cap: line, indent, inList, listStack } = inContainer);
            inContainer = containerStack.length ? containerStack.pop() : false;
          } else {
            // Process content normally but with admonition indent
            indent = inContainer.childIndent || "";
          }
          ...
          if ((grab = DELIMS[line]) === "v") {
            ...
            if (blockAttrs?.has("header-option")) {
              inTable = { header: true };
            } else if (blockAttrs?.has("noheader-option")) {
              inTable = { header: false };
            } else {
              inTable = { header: undefined };
            }
            ...
            if ((style = blockAttrs.get(1)) in ADMONS) {
              const admonitionText = ADMONS[style] + " _" + style + ":_";
              line = "> **" + admonitionText + "**";
              if (blockTitle) {
                line += " " + blockTitle.text;
              }
              inContainer.childIndent = "> ";
              inContainer.admonition = true;
              inContainer.cap = "";
              blockTitle = undefined;
          ...
          if (chr0 === "/" && line[1] === "/") {
            ...
            if (row) {
              ...
              // Apply URL conversion to table body cells only
              ...
              const shouldBeHeader = inTable.header === true ||
                (inTable.header === undefined &&
                  (inTable.cols ? cells.length - 1 === inTable.cols : cols === cells.length - 1) &&
                  (
                    // Original condition: empty line after header AND previous line not empty
                    (lines[idx + 1] === "" && lines[idx - 1] !== "") ||
                    // Block title makes it a header
                    blockTitle ||
                    // New condition: first content row after table start, followed by data rows
                    (lines[idx - 1] === "|===" &&
                     idx + 1 < lines.length &&
                     /^[a-z]*\|/.test(lines[idx + 1].trim()) &&
            ...
            if (!inList || inList.marker !== marker) {
              if ((listStack.length && ~(match = listStack.findIndex((it) => it.marker === marker)))) {
                ...
                let lindent = (attrs.lindent ??= parseInt(attrs.get("markdown-list-indent"), 10)) || (ordered ? 3 : 2);
                if (blockAttrs?.has("list-option")) {
                  lindent = parseInt(blockAttrs.get("list-option"), 10);
                }
            ...
            // If we're inside an admonition block, convert list items to blockquote format
            if (inContainer && inContainer.admonition) {
              line = text;
            } else {
          ...
          } else if (~(match = line.indexOf(": ")) && (style = line.substring(0, match)) in ADMONS) {
            // Check if this is a standalone admonition label (at start of line or preceded by whitespace)
            const beforeLabel = line.substring(0, match);
            const isStandalone = match < 10 || /^\s*$/.test(beforeLabel);

            if (isStandalone) {
              const admonitionText = line.substring(match + 2);
              if (admonitionText.trim()) {
                const processedText = applySubs.call(attrs, admonitionText);
                if (processedText.trim()) {
                  line = "> **" + ADMONS[style] + " _" + style + ":_** " + processedText;
                } else {
                  line = "> **" + ADMONS[style] + " _" + style + ":_**";
                }
              } else {
                line = "> **" + ADMONS[style] + " _" + style + ":_**";
              }
            ...
            // Check for standalone bold lines (e.g., "*Helpful Links*") and convert to h4 headers
            // Only apply if not in a list or paragraph context and the content is longer than 1 character
            const trimmedLine = line.trim();
            if (/^\*[^*]{2,}\*$/.test(trimmedLine) && !inList && !inPara) {
              // Extract the bold content and convert to h4 header
              const title = trimmedLine.replace(/^\*(.+?)\*$/, "$1");
              line = "#### " + title;
              inPara = false; // This is now a header, not a paragraph
            } else {
              if (blockAttrs?.has("hardbreaks-option")) {
                inPara = "hardbreaks";
              } else if (chr0 === ">" && line[1] === " ") {
                inPara = "> ";
              } else {
        ...
        if (line) {
          ...
          // Process footnotes in the line
          const footnoteResult = processFootnotes(line);
          line = footnoteResult.processed;

          accum[accum.length] = indent && line ? indent + line : line;

          // Add footnotes immediately after the line if it has footnotes
          if (footnoteResult.hasFootnotes) {
            accum[accum.length] = "";
            footnoteResult.footnotes.forEach((footnote) => {
              accum[accum.length] = footnote;
            });
          }
